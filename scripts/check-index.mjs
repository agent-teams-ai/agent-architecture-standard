...
if (JSON.stringify([...schemaIndex].toSorted()) !== JSON.stringify([...schemas].toSorted())) {throw new Error('schema index rows do not exactly match physical schemas');}
...
if (JSON.stringify([...registryIndex].toSorted()) !== JSON.stringify([...registryFiles].toSorted())) {throw new Error('registry index rows do not exactly match physical registries');}
...
if (JSON.stringify(profileEntries.toSorted()) !== JSON.stringify(profileArtifacts.toSorted())) {throw new Error('immutable profile-definition inventory is not indexed exactly');}
...
if (JSON.stringify([...vectorEntries].toSorted()) !== JSON.stringify([...vectorFiles].toSorted())) {throw new Error('manifest vector inventory does not exactly match physical vectors');}
...
if (JSON.stringify(vectorIndex.toSorted()) !== JSON.stringify(expectedVectorRoots.toSorted())) {throw new Error('vector index rows do not exactly match the normative suite roots');}
