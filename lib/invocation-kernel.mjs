...
import { assertKnownBudgets, assertLowerBounds, counterBudgets,
...
const exactKeys = (value, keys, label, fail) => {
  if (!value || typeof value !== 'object' || !sameCanonicalJson(Object.keys(value).toSorted(), [...keys].toSorted())) {fail(`${label} must contain the exact trusted field set`);}
};
...
const sortedRefs = (profiles) => profiles.map(profileRef).toSorted((left, right) => compareAscii(left.aasIdentity, right.aasIdentity));
const sortedCompleteRefs = (references) => references.map((reference) => structuredClone(reference))
  .toSorted((left, right) => compareAscii(left.aasIdentity, right.aasIdentity));
const failInvocationKernel = (message) => { throw new TypeError(`invalid AAS invocation kernel: ${message}`); };
...
export function createInvocationKernel({ operatorAuthority, targetAuthority, providerBudgets, ingressLimits } = {}, schemaAdmission) {
  const fail = failInvocationKernel;
  ...
  const preflightInvocation = function (rawRequestBytes) {
    ...
    const requestKeys = request.targets.map(({ id }) => id).toSorted();
    if (!sameCanonicalJson(Object.keys(returned).toSorted(), requestKeys)) {fail('resolver target-selection keys must exactly equal request target keys');}
