...
const isJsonWhitespace = (codePoint) => codePoint === 0x20 || codePoint === 0x09 || codePoint === 0x0a || codePoint === 0x0d;
...
export function parseStrictJson(input, limits = DEFAULT_JSON_LIMITS) {
  ...
  const ws = () => { while (at < text.length && isJsonWhitespace(text.charCodeAt(at))) {at++;} };
  const string = () => {
    const start = at++;
  ...
  const value = (depth) => {
    ...
    if (match) {
      ...
      // Bound resources by the input spelling rather than imposing an
      // unrelated mathematical-scale ceiling.
      // Keep the exponent as an integer whose storage is bounded by the input
      // itself.  We never construct 10^N until N is already proven no larger
      // than the coefficient, so long but mathematically harmless spellings do
      // not trigger an undocumented scale ceiling or expansive allocation.
      const scale = BigInt(exponentText) - BigInt(fraction.length);
      let integer;
      if (scale >= 0n) {
        if (scale > 16n || BigInt(coefficientText.length) + scale > 16n) {fail(DiagnosticCode.INVALID_NUMBER, 'number is outside the safe-integer profile');}
        integer = BigInt(coefficientText) * (10n ** scale);
      } else {
        const places = -scale;
        if (places > BigInt(coefficientText.length)) {fail(DiagnosticCode.INVALID_NUMBER, 'number is not mathematically integral');}
        const split = coefficientText.length - Number(places);
        if (!/^0*$/u.test(coefficientText.slice(split))) {fail(DiagnosticCode.INVALID_NUMBER, 'number is not mathematically integral');}
        integer = BigInt(coefficientText.slice(0, split) || '0');
      }
