...
export function parseStrictJson(input, limits = DEFAULT_JSON_LIMITS) {
  ...
  const value = (depth) => {
    ...
    if (match) {
      ...
      if (!Number.isFinite(number) || !Number.isSafeInteger(number) || Object.is(number, -0)) fail(DiagnosticCode.INVALID_NUMBER, 'decoded number is outside the safe-integer profile');
      return number;
    }
