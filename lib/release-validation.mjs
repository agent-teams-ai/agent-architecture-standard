...
export function assertReleaseCohortsNonReuse(...manifests) {
  ...
  const isRc = (manifest) => manifest.members.every(({ version }) => /-rc\.[1-9][0-9]*$/u.test(version));
  const rc = manifests.find(isRc), numeric = manifests.find((manifest) => !isRc(manifest));
  if (!rc || !numeric) fail('one RC cohort and one numeric cohort are required');
  const rcNames = rc.members.map(({ name }) => name).sort();
  const numericNames = numeric.members.map(({ name }) => name).sort();
