...
const failReleaseManifest = (message) => { throw new TypeError(`invalid AAS release manifest: ${message}`); };
const failReleaseCohorts = (message) => { throw new TypeError(`invalid AAS release cohorts: ${message}`); };
const isRcCohort = (manifest) => manifest.members.every(({ version }) => /-rc\.[1-9][0-9]*$/u.test(version));
...
export function computeReleaseManifestAasIdentity(manifest) {
  return computeFramedIdentity('releaseManifest', manifest, releaseManifestIdentityProjection);
}

export function assertReleaseManifestInvariants(manifest) {
  const fail = failReleaseManifest;
  ...
  const rc = versions.every((version) => /^0\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)-rc\.[1-9][0-9]*$/u.test(version));
  ...
  const exactVersions = [...new Set(versions)].toSorted();
  if (canonicalize([...manifest.supportedVersions].toSorted()) !== canonicalize(exactVersions)) {fail('supportedVersions must exactly equal member versions');}
  const edges = new Set();
...
export function assertReleaseCohortsNonReuse(...manifests) {
  const fail = failReleaseCohorts;
  if (manifests.length !== 2) {fail('exactly one paired RC/numeric cohort sequence is required');}
  ...
  const rc = manifests.find(isRcCohort), numeric = manifests.find((manifest) => !isRcCohort(manifest));
  if (!rc || !numeric) {fail('one RC cohort and one numeric cohort are required');}
  const rcNames = rc.members.map(({ name }) => name).toSorted();
  const numericNames = numeric.members.map(({ name }) => name).toSorted();
  if (canonicalize(rcNames) !== canonicalize(numericNames)) {fail('paired cohorts must have identical member names');}
  const numericByName = new Map(numeric.members.map((member) => [member.name, member]));
  for (const member of rc.members) {
    const base = member.version.replace(/-rc\.[1-9][0-9]*$/u, '');
    if (numericByName.get(member.name)?.version !== base) {fail(`RC base version does not correspond to numeric version: ${member.name}`);}
  }
  return manifests;
}
