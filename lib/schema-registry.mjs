...
import { assertNoEmbeddedSchemaResources, isCanonicalSchemaId } from './schema-graph.mjs';
...
export class OfflineSchemaRegistry {
  ...
  add(schema) {
    assertNoEmbeddedSchemaResources(schema);
    const id = schema.$id;
    if (typeof id !== 'string') {throw new AasDiagnosticError(DiagnosticCode.SCHEMA_INVALID, 'schema has no $id');}
    if (!isCanonicalSchemaId(id)) {throw new AasDiagnosticError(DiagnosticCode.SCHEMA_ALIAS, `schema ID is not canonical: ${id}`);}
  ...
  ids() { return [...this.schemas.keys()].toSorted(); }
}
