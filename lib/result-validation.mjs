...
export function assertResultInvariants(result, request, rawInputBytes) {
  ...
  for (const diagnostic of result.diagnostics) {
    ...
    const trace = diagnostic.decisionTrace;
    if (trace.bindingAasIdentity !== header.bindingAasIdentity) fail(`diagnostic trace/header binding mismatch: ${diagnostic.targetId}`);
    const candidateIds = trace.candidateBindings.map((candidate) => candidate.id);
    if (new Set(candidateIds).size !== candidateIds.length) fail(`duplicate diagnostic candidate binding ID: ${diagnostic.targetId}`);
    const selected = trace.candidateBindings.filter((candidate) => candidate.id === trace.selectedBindingId);
    if (selected.length !== 1) fail(`selected diagnostic binding must identify exactly one candidate: ${diagnostic.targetId}`);
    if (selected[0].aasIdentity !== trace.bindingAasIdentity) fail(`selected diagnostic candidate identity mismatch: ${diagnostic.targetId}`);
