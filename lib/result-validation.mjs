...
export const RESOURCE_ACCOUNTING_PROFILE_AAS_IDENTITY = 'aas:v0:sha256:2a9d4536b7e074431ae08e604fd7dcc3c790f3bebe42ffa511567b7196077249';
...
const deriveBindingDecision = (bindings, coordinate, fail) => {
  ...
  if (tied.some((binding) => !sameJson(bindingSemantics(binding), bindingSemantics(tied[0])))) fail(`aas.problem.binding-set-conflict: ${coordinate.targetId}`);
...
const expectedAnalysisKeyProjection = (request, analysisKey) => ({
  operationProfile: analysisKey.operationProfile,
  evaluatorProfiles: analysisKey.evaluatorProfiles,
  inputAasIdentities: analysisKey.inputAasIdentities,
  budgets: analysisKey.budgets,
  accountingProfile: analysisKey.accountingProfile,
  semanticExtensions: analysisKey.semanticExtensions,
  requestAasIdentity: analysisKey.requestAasIdentity,
  operation: analysisKey.operation,
  profileAasIdentity: analysisKey.profileAasIdentity,
  analyzerAasIdentity: analysisKey.analyzerAasIdentity,
  snapshotAasIdentity: analysisKey.snapshotAasIdentity
});

/** Pre-evaluation validation and derivation. This function must complete before evaluator work begins. */
export function preflightInvocation({ request, analysisKey, bindingSet, targetSelectionsByTarget, applicableBindingsByTarget }, rawInputBytes) {
  const fail = (message) => { throw new TypeError(`invalid AAS invocation: ${message}`); };
  assertRequestInvariants(request, rawInputBytes);
  if (!analysisKey || !bindingSet || !Array.isArray(bindingSet.bindings) || !targetSelectionsByTarget || typeof targetSelectionsByTarget !== 'object') fail('analysis key, complete binding set, and target selections are required');
  ...
  if (computeBindingSetAasIdentity(bindingSet) !== bindingSet.aasIdentity) fail('binding-set aasIdentity does not match the normative projection');
  if (bindingSet.bindings.length > 10000) fail('binding set exceeds 10000 members');
  for (const binding of bindingSet.bindings) if (computeBindingAasIdentity(binding) !== binding.aasIdentity) fail(`binding aasIdentity does not match the normative projection: ${binding.id}`);
  const inputIdentities = [request.snapshotAasIdentity, ...request.targets.map((target) => target.input.aasIdentity)];
  const semanticExtensions = { request: request.extensions, criticalRequest: request.criticalExtensions,
    targets: request.targets.map(({ id, extensions, criticalExtensions }) => ({ id, extensions, criticalExtensions })) };
  const expectedAnalysis = { ...expectedAnalysisKeyProjection(request, analysisKey), requestAasIdentity: request.aasIdentity,
    operation: request.operation, profileAasIdentity: request.profileAasIdentity, analyzerAasIdentity: request.analyzerAasIdentity,
    snapshotAasIdentity: request.snapshotAasIdentity, inputAasIdentities: inputIdentities, semanticExtensions };
  if (!sameJson(expectedAnalysisKeyProjection(request, analysisKey), expectedAnalysis)) fail('analysis key semantic projection is unrelated to the request');
  if (analysisKey.operationProfile?.aasIdentity !== request.profileAasIdentity) fail('analysis key operation profile is not the request profile');
  if (!Array.isArray(analysisKey.evaluatorProfiles) || analysisKey.evaluatorProfiles.length === 0
    || !analysisKey.evaluatorProfiles.some(({ aasIdentity }) => aasIdentity === request.analyzerAasIdentity)) fail('analysis key evaluator profiles do not bind the request analyzer');
  const profileIdentity = request.accountingProfileAasIdentity;
  if (profileIdentity !== RESOURCE_ACCOUNTING_PROFILE_AAS_IDENTITY) fail('accounting profile identity is not the authoritative registered profile definition');
  ...
  if (byIdentity.size !== bindings.length || new Set(bindings.map(({ id }) => id)).size !== bindings.length) fail('duplicate binding-set member ID or identity');
  ...
  if (applicableBindingsByTarget !== undefined && (!applicableBindingsByTarget || typeof applicableBindingsByTarget !== 'object' || !sameJson(Object.keys(applicableBindingsByTarget).sort(), requestKeys))) fail('applicable binding projection keys must exactly equal request target keys');
  ...
  for (const target of request.targets) {
    ...
    for (const binding of derived.applicable) if (binding.accountingProfile?.aasIdentity !== profileIdentity) fail(`accounting profile substitution in binding: ${target.id}/${binding.id}`);
    if (!sameJson(derived.decision, target.bindingSelection)) fail(`request binding selection is not the complete deterministic binding-set derivation: ${target.id}`);
    if (applicableBindingsByTarget !== undefined) {
      const projection = applicableBindingsByTarget[target.id];
      if (!Array.isArray(projection) || !sameJson(projection.map(bindingCandidateProjection), derived.applicable.map(bindingCandidateProjection))) fail(`caller applicable binding map is not the exact checked projection: ${target.id}`);
    }
    if (derived.selected) {
      budgetSources.push(derived.selected.budgets);
      selectedByTarget[target.id] = derived.selected;
      rolloutByTarget[target.id] = derived.rolloutDisposition;
    }
    budgetSources.push(target.input.limits);
  }
  const effectiveBudgets = deriveEffectiveBudgets(budgetSources);
  return deepFreeze(structuredClone({ request, analysisKey, bindingSet, effectiveBudgets, selectedByTarget, rolloutByTarget }));
}

/** Post-result reconciliation reuses the completed preflight derivation. */
export function reconcileInvocationResult(preflight, result, rawInputBytes) {
  const fail = (message) => { throw new TypeError(`invalid AAS invocation result: ${message}`); };
  if (!preflight?.effectiveBudgets) fail('completed preflight derivation is required');
  assertResultInvariants(result, preflight.request, rawInputBytes);
  ...
  for (const target of preflight.request.targets) {
    ...
    if (selected && resolution.diagnostic.rolloutDisposition !== preflight.rolloutByTarget[target.id]) fail(`selected binding rollout disposition mismatch: ${target.id}`);
  }
...
export function assertInvocationInvariants(invocation, rawInputBytes) {
  ...
  return reconcileInvocationResult(preflight, invocation.result, rawInputBytes);
}
