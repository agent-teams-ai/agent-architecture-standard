...
/** Canonical request bytes charged to maxInputBytes (the request identity projection). */
export const canonicalRequestInputBytes = (request) => canonicalBytes(requestIdentityProjection(request));

/** Canonical result bytes charged to maxOutputBytes; zeroing the field prevents a length fixed point. */
export const canonicalResultOutputBytes = (result) => {
  const projection = resultIdentityProjection(result);
  projection.realizedCounters.outputBytes = 0;
...
export function assertRequestResultInvariants(request, result) {
  ...
  if (result.realizedCounters.inputBytes !== canonicalRequestInputBytes(request)) fail('realized inputBytes does not match canonical request accounting');
...
export function assertRequestInvariants(request) {
  ...
  if (canonicalRequestInputBytes(request) > request.budgets.maxInputBytes) fail('canonical request input exceeds maxInputBytes');
