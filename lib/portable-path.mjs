...
const isForbiddenCodePoint = (codePoint) => codePoint <= 0x1f
  || (codePoint >= 0x7f && codePoint <= 0x9f)
  || codePoint === 0x061c
  || (codePoint >= 0x200e && codePoint <= 0x200f)
  || (codePoint >= 0x202a && codePoint <= 0x202e)
  || (codePoint >= 0x2066 && codePoint <= 0x2069);
const containsForbiddenCodePoint = (value) => [...value].some((character) => isForbiddenCodePoint(character.codePointAt(0)));
const failPortablePath = (message) => { throw new TypeError(`invalid portable path: ${message}`); };
...
export function normalizeUnicode17Nfc(value) {
  ...
  for (const character of value) {
    ...
    for (const cp of expanded) {
      ...
      while (position > 0) {
        if (ccc === 0 || (CCC.get(ordered[position - 1]) ?? 0) <= ccc) {break;}
        position -= 1;
      }
...
export function assertPortablePath(value) {
  ...
  if (containsForbiddenCodePoint(value)) {fail('control or bidi-control code point is forbidden');}
