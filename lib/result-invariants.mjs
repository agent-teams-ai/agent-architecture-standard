...
const failRequest = (message) => { throw new TypeError(`invalid AAS request: ${message}`); };
const failResult = (message) => { throw new TypeError(`invalid AAS result: ${message}`); };
const failRequestResultPair = (message) => { throw new TypeError(`invalid AAS request/result pair: ${message}`); };
...
export function assertResultInvariants(result, request, rawInputBytes) {
  ...
  for (const resolution of result.resolutions) {
    ...
    const expectedHighest = [...severities].toReversed().find((severity) => actualCounts[severity] > 0) ?? 'none';
...
export function assertRequestResultInvariants(request, result, rawInputBytes) {
  const fail = failRequestResultPair;
  ...
  if (!sameCanonicalJson([...actualDispositions.keys()].toSorted(), [...expectedDispositions.keys()].toSorted())) {fail('extension dispositions must form the exact request/target/location/extension bijection');}
  return result;
}
